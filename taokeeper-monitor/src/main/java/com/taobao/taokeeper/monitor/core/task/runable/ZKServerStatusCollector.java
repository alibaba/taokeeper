...
public class ZKServerStatusCollector implements Runnable {
    ...
    public void run() {
        try {
            ...
            monitorZooKeeperRTStats((ZooKeeperStatusV2) zooKeeperStatus, zookeeperCluster.getClusterId());
    ...
    private void monitorZooKeeperRTStats(ZooKeeperStatusV2 zooKeeperStatus, int clusterId) {
        Map<String, String> rtStatus = ZooKeeperRTCollectJob.getRtStatus().get(clusterId);

        if (rtStatus == null) return;

        ZooKeeperStatusV2.RTStats rtStats = new ZooKeeperStatusV2.RTStats();
        rtStats.setCreate(Long.parseLong(rtStatus.get("create")));
        rtStats.setDelete(Long.parseLong(rtStatus.get("delete")));
        rtStats.setSetData(Long.parseLong(rtStatus.get("setData")));
        rtStats.setGetData(Long.parseLong(rtStatus.get("getData")));
        rtStats.setGetChildren(Long.parseLong(rtStatus.get("getChildren")));
        rtStats.setExists(Long.parseLong(rtStatus.get("exists")));
        rtStats.setCreateSession(Long.parseLong(rtStatus.get("createSession")));

        zooKeeperStatus.setRtStats(rtStats);
    }
